...
export const EditTaskModels = ({ model, close }) => {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            taskTheme: model.taskTheme._id
        },
        validationSchema: validationSchema,
        onSubmit: (values) => {
            // console.log(values);
            dispatch(updateTaskModel({ _id: model._id, name: values.name, description: values.description, taskTheme: values.taskTheme }));
            close();
        }
    });
    return (
        <div>
            <h1 style={{ marginBottom: 20 }}>Create task theme</h1>
