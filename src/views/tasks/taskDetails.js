...
import {updatetask,detailstask } from '../../store/actions/taskActions'
...
export const TaskDetails = () => {
 ...
 const { departements } = departementList;
  ...
  const { loading, error, tasks } = taskDetails; 
  ...
  const {loading: loadingUpdate, error: errorUpdate, success: successUpdate} = taskUpdate;
   ...
   useEffect(() => {
    ...
    useEffect(() => {
        ...
        if(!tasks){
           dispatch(detailstask(id));
         console.log(tasks);
         setTask(taskDetails.tasks);
      }
    
    const formik = useFormik({
        initialValues: {
            name: tassk?.name,
            description: tassk?.description,
            week:tassk?.week._id,
            user: tassk?.user._id,
            start: tassk?.startDate,
            endDate: tassk?.endDate,
            taskModel:tassk?.taskModel._id,
            component: tassk?.component._id,
            departement:tassk?.departement._id,
            taskState:tassk?.taskState._id,
            ...
            <form onSubmit={formik.handleSubmit}>
             ...
             <FormControl fullWidth>
                ...
                <div style={{ marginTop: 20, float: 'right' }}>
                    ...
                    <Button color="error" variant="contained" style={{ marginLeft: 5 }} >
                        Cancel
                    </Button>
                </div>
            </form>
